...
def compute_metrics(
    ...
    for pred_grom, class_id in zip(pred_geoms, pred_classes):
        pred_grom.class_id = class_id
    ...
    for gt_geom, gt_class in zip(gt_geoms, gt_classes):
        ...
        ious = [iou(m, gt_geom) for m in matches]
        ious = [iou for iou in ious if iou > iou_thresh]
        if len(ious) > 0:
