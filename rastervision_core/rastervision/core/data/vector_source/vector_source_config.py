...
def vector_source_config_upgrader(
    ...
    if version == 4:
        ...
        class_inf_tf = ClassInferenceTransformerConfig(
            ...
            class_id_to_filter=cfg_dict.get('class_id_to_filter'),
        ).dict()

        line_bufs = (
            {}
            if cfg_dict.get('line_bufs') is None
            else cfg_dict.get('line_bufs')
        )
        point_bufs = (
            {}
            if cfg_dict.get('point_bufs') is None
            else cfg_dict.get('point_bufs')
        )
        buf_tfs = [
            BufferTransformerConfig(
...
class VectorSourceConfig(Config):
    ...
    @abstractmethod
    def build(
        self, class_config: 'ClassConfig', crs_transformer: 'CRSTransformer'
    ) -> 'VectorSource':
        pass
